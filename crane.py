...
class Crane(object):
    ...
    def __init__(self, clientaddress):
        ...
        self._node_trolley_direction_forward_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Trolley.ControlSignals.Direction1_Request")

        self._node_trolley_direction_backwards_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Trolley.ControlSignals.Direction2_Request")
        self._node_trolley_speed_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Trolley.ControlSignals.SpeedRequest")
        self._node_trolley_speed_feedback = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Trolley.ControlSignals.SpeedFeedback")
        self._node_trolley_speed_feedback_mmin = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Trolley.ControlSignals.SpeedFeedback_mmin")


        # Not currently useds
        self._node_bridge_direction_forward_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Bridge.ControlSignals.Direction1_Request")

        self._node_bridge_direction_backward_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Bridge.ControlSignals.Direction2_Request")
        self._node_bridge_speed_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Bridge.ControlSignals.SpeedRequest")
        self._node_brige_speed_feedback = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Bridge.ControlSignals.SpeedFeedback")
        self._node_brige_speed_feedback_mmin = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Bridge.ControlSignals.SpeedFeedback_mmin")


        # Not currently used
        self._node_hoist_direction_up_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Hoist.ControlSignals.Direction1_Request")

        self._node_hoist_direction_down_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Hoist.ControlSignals.Direction2_Request")
        self._node_hoist_speed_request = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Hoist.ControlSignals.SpeedRequest")
        self._node_hoist_speed_feedback = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Hoist.ControlSignals.SpeedFeedback")
        self._node_hoist_speed_feedback_mmin = self.client.get_node(
            NS + ";s=SCF.PLC.DX_Custom_V.Status.Hoist.ControlSignals.SpeedFeedback_mmin")
    ...
    def set_watchdog(self, watchdog_value):
        ...
        self._node_watchdog.set_value(ua.DataValue(ua.Variant(watchdog_value, ua.VariantType.Int16)))
    ...
    def set_trolley_speed(self, speed):
        ...
        self._node_trolley_speed.set_value(ua.DataValue(ua.Variant(
            speed, ua.VariantType.Float)))


    def set_bridge_speed(self, speed):
        """Set speed Bridge in % 0-100"""
        self._node_bridge_speed.set_value(ua.DataValue(ua.Variant(
            speed, ua.VariantType.Float)))


    def set_hoist_speed(self, speed):
        """Set speed Hoist in % 0-100"""
        self._node_hoist_speed.set_value(ua.DataValue(ua.Variant(
            speed, ua.VariantType.Float)))
    ...
    def get_trolley_speed_request(self):
        ...
        inv = -1 if self._node_trolley_direction_backwards_request.get_value() else 1
    ...
    def get_bridge_speed_request(self):
        """ Get requested/target speed (ref), return negative if request is backward """
        inv = -1 if self._node_bridge_direction_backward_request.get_value() else 1
